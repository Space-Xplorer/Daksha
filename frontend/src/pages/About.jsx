...
const About = () => {
  const values = [
    { 
      ...
      icon: <Target className="text-[#F4C2C2]" /> 
    },
    { 
      ...
      icon: <Users className="text-[#F4C2C2]" /> 
    },
    { 
      ...
      icon: <Sparkles className="text-[#F4C2C2]" /> 
    }
  ];
      ...
      <div className="grid lg:grid-cols-3 gap-8 mb-40">
        {values.map((val, i) => (
          <GlassCard key={i} className="p-10 border-b-8 border-[#4B0082]/10 hover:border-[#4B0082] transition-all duration-500 group">
            ...
            <h4 className="text-xl font-black text-[#4B0082] mb-4 uppercase italic">{val.title}</h4>
        ...
        <div>
           ...
           <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-[#4B0082]">
                   <div className="w-1.5 h-1.5 bg-[#F4C2C2] rounded-full" /> {item}
                </li>
              ))}
           </ul>
        </div>
