...
It provides both a CLI interface and Flask application factory.
...
Usage:
    ...
    python main.py workflow     # Run workflow with custom data
...
from routes import auth, applications, workflow, admin, health
...
def create_app(config=None):
    ...
    app.register_blueprint(health.bp)  # Health check routes
...
def run_api(args):
    ...
    # PRODUCTION: Validate configuration if in production mode
    flask_env = os.getenv('FLASK_ENV', 'development')
    if flask_env == 'production':
        print("🔍 Running production configuration validation...\n")
        from src.utils.config_validator import validate_production_config
        if not validate_production_config():
            print("\n⛔ Cannot start server with invalid production configuration.")
            print("Please fix the errors above or set FLASK_ENV=development\n")
            sys.exit(1)
    
    app = create_app()
    
    host = os.getenv('API_HOST', '0.0.0.0')
    port = int(os.getenv('PORT', args.port if hasattr(args, 'port') else 8000))
    debug = (hasattr(args, 'debug') and args.debug) or os.getenv('FLASK_ENV') == 'development'
    ...
    - Model Status: http://{host}:{port}/api/health/models
    - Full Health: http://{host}:{port}/api/health/all
    ...
    # Disable reloader to prevent crashes during workflow execution
    # Set debug=False to completely disable file watching
...
def show_info(args):
    ...
    Quick Start:
    1. python main.py validate    # Validate system
    2. python main.py api         # Start API server
    3. python main.py demo --auto # Run demo
...
Examples:
  ...
  python main.py api --port 8000        # Start on custom port
  python main.py demo                   # Run interactive demo
  python main.py demo --auto            # Run automated demo
  python main.py test                   # Run all tests
  python main.py test --unit            # Run unit tests only
  python main.py validate               # Validate system
  python main.py workflow               # Run workflow with defaults
  python main.py workflow --data data.json  # Run with custom data
  python main.py info                   # Show system info
    ...
    if args.command == 'api':
        ...
        # Default: Start API server if no command specified
        print("No command specified. Starting API server...\n")
        
        # Create a simple args object with defaults
        class DefaultArgs:
            port = 8000
            debug = False
