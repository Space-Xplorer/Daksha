...
def demo_rejection_case():
    """
    Demo function for rejection scenario (low CIBIL).
    """
    print_section("BONUS DEMO: REJECTION SCENARIO", "93")
    print("Demonstrating: Low CIBIL Score Rejection")
    print("\nPress Enter to begin...")
    input()
    
    applicant_data = {
        "cibil_score": 550,  # Below minimum
    ...
    state = create_initial_state(
        ...
        submitted_dob="1995-01-01"
    )
    
    print_info("Running workflow with low CIBIL score...")
    ...
    try:
        ...
        if not final_state.get("compliance_passed"):
            print_error("REJECTED: Compliance violations detected")
            for violation in final_state.get("compliance_violations", []):
                print(f"  - {violation.get('rule')}: {violation.get('reason')}")
                print(f"    Severity: {violation.get('severity')}")
        
        print("\n💡 This demonstrates:")
        print("  ✓ Automatic compliance enforcement")
        print("  ✓ Clear rejection reasons")
        print("  ✓ Regulatory protection")
...
if __name__ == "__main__":
    ...
    print("  1. Complete workflow with Rajesh Kumar (APPROVAL case)")
    print("  2. Rejection scenario (BONUS)")
    print()
    ...
    if success:
        print("\n" + "="*80)
        response = input("\nWould you like to see the rejection scenario? (y/n): ")
        if response.lower() == 'y':
            demo_rejection_case()
